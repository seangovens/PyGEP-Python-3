...
class Population(object):
    ...
    about two loci per chromosome.  Example Population usage::
        ...
        for _ in range(50): # maximum number of generations
            if p.best.solved:
                break
    ...
    def __init__(self, cls, size, head, genes=1, linker=default_linker):
        ...
        self.population = [i for _, i in zip(range(size),
    ...
    def cycle(self):
        ...
        if self.mean > 0:
            ...
            select = [random.random() * scaling for _ in range(self.size-1)]
            select.sort()
    ...
    def _pairs(self, rate):
        ...
        if rate and self.size >= 3:
            ...
            orgs = [i for i in range(1, self.size) if random.random() < rate]
            random.shuffle(orgs)
