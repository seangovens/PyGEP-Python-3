...
class MetaChromosome(type):
    ...
    def __new__(mcs, name, bases, dct):
        ...
        # Find the max arity
        try:
            typ.arity = max([f.__code__.co_argcount for f in typ.functions])
        except ValueError:
            typ.arity = 0
...
class Chromosome(object, metaclass=MetaChromosome):
    '''
        ...
        while True:
            ...
            for i in range(genes):
                new_genes[i] = cls.gene_type(
                    [random.choice(cls.symbols)   for _ in range(head)] + \
                    [random.choice(cls.terminals) for _ in range(tail)], head
                )
    ...
    def __cmp__(self, other):
        ...
        return 1 if self.fitness > other.fitness else \
            0 if self.fitness == other.fitness else -1
    ...
    def invert(self):
        ...
        i = random.choice(range(len(self.genes)))
        start, stop = random.sample(range(self.head), 2)
    ...
    def transpose_ris(self, length):
        ...
        try:
            start = random.choice(
                [i for i in range(len(source)) if callable(source[i])]
            )
    ...
    def crossover_one_point(self, other):
        ...
        gene  = random.choice(range(len(genes1)))
        index = random.choice(range(len(genes1[gene])))
    ...
    def crossover_two_point(self, other):
        ...
        ind1, ind2 = random.sample(range(len(self)), 2)
        if ind1 > ind2:
            ind1, ind2 = ind2, ind1
